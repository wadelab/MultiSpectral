...
while((experimentType<1) || (experimentType>3))
    experimentTypeS=input ('Experiment condition code (L, M, S, LMS, LM): ','s'); %
...
while ((k<trialsDesired) && (response ~= -1))
    ...
    response=led_doLEDTrial_5LEDs_16bit(dpy,stim,q,s); % This should return 0 for an incorrect answer and 1 for correct
    disp(response)
