...
byte LEDampSign[5]; // Explicitly set number of ampSign values (nLEDS) - indicate the sign of the amp, here 0 is positive and 1 is a negative
...
byte pulseDuration[1]; // How long each pulse lasts in ms
...
// This runs forever  
  ...
  // Initially we read everything into byte arrays, then do the maths at the end.
      ...
      if (Serial.available() > 0) {
          ...
          Serial.readBytes(modulationRateHz16Bit,2); // penultimate 2 bytes are the frequency of the flicker in Hz multiplied by 256. We do this to allow fractional flicker rates.
          Serial.readBytes(pulseDuration,1); // last byte is the pulse duration
