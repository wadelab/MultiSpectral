...
load('LEDspectra_220416.mat'); %load in calib for the prizmatix
...
%resample LEDcalib
LEDspectra=interp1(LEDcalib(:,1),LEDcalib(:,2:end),Dummydpy.WLrange);


LEDsToUse=[1,2,3,4,5]; 
Dummydpy.LEDsToUse = LEDsToUse;
Dummydpy.ConeTypes='LMS';
coneSpectra=creatingLMSspectra(Dummydpy); %outputs the L L' M S spectra, with first column containing wavelengths

Dummydpy.coneSpectra = coneSpectra;
Dummydpy.coneSpectra(isnan(Dummydpy.coneSpectra))=0;
Dummydpy.coneSpectra=Dummydpy.coneSpectra(:,2:end); %remove the wavelengths column
