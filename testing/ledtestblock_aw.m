...
dpy.backRGB.dir=[1 1 1 0.37]; %  this is the scale applied to the background LEDs. I >think< that 1111 means they all have the same voltage out on them at the midpoint. 
...
expt.blockDurSecs=20; %Duration of stimulus presentation, secs
expt.ISIsecs=3; %Inter stimulus interval, secs
expt.preStimSecs=1; %Pre-stimulus pause before the start of the trials, secs
expt.stim.chrom.stimLMS.dir=[0 1 0 0]; %This defines all the colour directions that we want to test (luminance, Scone isolation)
expt.stim.chrom.stimLMS.scale=[0.007]; % these are the contrasts that correspond to the the above directions
expt.stim.temporal.freq=[2]; % each of these frequencies will be applied to each of the colours, 0freq will create a control (off-) stim
% expt.stim.chrom.noise.stimLMS.dir=[1 1 1];
% expt.stim.chrom.noise.stimLMS.scale=0.03;
% expt.stim.chrom.noise.type='white';
% expt.stim.chrom.noise.temporal=[0.1:20]
...
for thisColour=1:nColours;
    for thisFreq=1:nFreqs;
        ...
        %trial(trialIndex).stimulus.noise=expt.stim.chrom.noise;
