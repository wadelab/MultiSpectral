...
% Last Modified by GUIDE v2.5 30-Jul-2013 11:08:57
...
selected_SCANstring = str2num(string_SCANlist{SessionNum}); 
assignin('base','SessionNum',selected_SCANstring);
...
% --- Executes on selection change in StimCode.
function StimCode_Callback(hObject, eventdata, handles)
StimCode=get(hObject,'Value');
string_SCANlist = get(hObject,'String');
selected_SCANstring = (string_SCANlist{StimCode}); 
assignin('base','StimCode',selected_SCANstring);
% hObject    handle to StimCode (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns StimCode contents as cell array
%        contents{get(hObject,'Value')} returns selected item from StimCode
...
% --- Executes on key press with focus on SessionNumber and none of its controls.
function SessionNumber_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to SessionNumber (see GCBO)
