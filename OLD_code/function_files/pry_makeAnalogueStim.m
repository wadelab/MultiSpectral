function stim=pry_makeAnalogueStim(dpy,session,expt)
...
elseif (isfield(dpy,'peaks'))
    error('Must use real calibration for the LEDs');
...
sampleRate=expt.stim.temporal.sampleRate;
duration=expt.stim.temporal.duration;
nPoints=sampleRate*duration; % Total number of data points to create.
freq=expt.stim.temporal.freq; % In Hz
duration=expt.stim.temporal.duration;
scale=expt.stim.chrom.stimLMS.scale;

modulation=sin(linspace(0,2*pi*freq*duration,nPoints)); % Sine wave function
expt.stim.chrom.stimLMS.cont=modulation*scale;
 

[LEDStim]= pry_sensor2primary(dpy,expt.stim.chrom.stimLMS,dpy.backRGB,coneSpectra,expt);
